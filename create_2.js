...
if (paymentPlanStatusName === "Subscription") {
    ...
    if (shouldDivideEqually === 'Equally') {
        ...
        output.text(`The total price of the product is: £${totalAmount}. The custom amounts must equal the total price of the product`);
        ...
        for (let i = 0; i < paymentLength; i++) {
            ...
            if (totalCustomAmount === totalAmount) {
                let choice = await input.buttonsAsync('Total inputs equal product price. Do you want to add more payments or exit?', ['Add More', 'Exit']);
                if (choice === 'Exit') {
                    break;
                }
            }
